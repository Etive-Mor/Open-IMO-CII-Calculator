...
# Utility to convert CalculationResult and ResultYear to dict for JSON serialization
...
def single_fuel_demo():
    print("---------------------")
    print("Generating a single-fuel ship report...")
    print("---------------------")

    calculator = ShipCarbonIntensityCalculator()
    fuel_consumption_megatons = 19_000
    result = calculator.calculate_attained_cii_rating(
        ...
        fuel_type_consumptions=[FuelTypeConsumption(TypeOfFuel.DIESEL_OR_GASOIL, fuel_consumption_megatons * 1_000_000)],
        target_year=2019
    )
    ...
    out_path = os.path.join("demo-results", f"{run_id}-single-fuel-demo.json")
    with open(out_path, "w") as f:
        json.dump(result_dict, f, indent=2, default=str)
    print(f"Result written to {out_path}")
    print("---------------------")
    print("Completed the single-fuel ship report...")
    print("---------------------")
